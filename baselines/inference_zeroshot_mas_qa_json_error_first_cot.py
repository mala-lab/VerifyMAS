...
Your task is to analyze a multi-agent conversation trajectory and identify faulty agent-error pairs.
You must follow an ERROR-FIRST diagnostic process:

1. First detect what error type(s) occur in the trajectory.
2. Then attribute each detected error type to the agent who introduced, caused, or most directly contributed to that error.

This is important: do NOT start by judging each agent independently.
You should first reason about whether each error type exists at the trajectory level, and only then localize the responsible agent.
...
## ANALYSIS WORKFLOW
Follow these steps internally only. Do not output the reasoning.

### Step 1: Trajectory-Level Understanding
First, understand the whole trajectory globally:
- Identify the original user query or task requirement.
- Understand the intended goal of the multi-agent collaboration.
- Track the main decisions, dependencies, and final outcome.
- Note whether the failure is local to one step or only visible from the full trajectory.

### Step 2: Error-First Detection
Next, detect which error type(s) appear in the trajectory.

For each error definition:
- Check whether the trajectory provides clear or reasonably strong evidence for this error.
- Focus on the full trajectory, not only isolated local logs.
- Consider whether the error negatively affects, or is likely to affect, task completion, final answer quality, coordination, or verification.
- Do not select an error type based only on weak, vague, or speculative evidence.
- If no error type is clearly supported, output an empty faulty_agents list.

Important:
- First decide the error type.
- Do not assign an agent before deciding whether the error exists.
- Avoid over-predicting errors.
- Prefer no prediction over a weakly supported prediction.

### Step 3: Agent Attribution for Each Detected Error
For each detected error type, identify the responsible agent.

The responsible agent should be the one who:
- Introduced the error,
- Directly caused the error,
- Failed to perform a required action,
- Misled later agents,
- Ignored important previous context,
- Or made the decision that most clearly led to the failure.

Attribution rules:
- Use the exact agent name as it appears in the conversation.
- Attribute the error to the most directly responsible agent.
- If multiple agents clearly committed the same error, output multiple pairs.
- If an error exists but the responsible agent is unclear, do not output that pair.
- If another agent merely follows a previous wrong instruction, prefer the agent who introduced the original error.
- Do not hallucinate agent names that do not appear in the conversation.

### Step 4: Final Judgment
Construct the final answer as a list of faulty agent-error pairs:
- Each pair must contain one agent_name and one error_type.
- Remove duplicate pairs.
- Only include pairs with clear evidence.
- If no clear faulty pair exists, output an empty list.
